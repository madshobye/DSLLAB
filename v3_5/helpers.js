...
function windowResized() {
  let canvasContainer = select("#canvasdiv");
  ...
  //select('#canvascont').width-50, select('#canvascont').height-1);
  // canvasElement.width = select('#canvascont').width;
  // print(select('#canvascont').width);
}
...
function calculateBezierCurveLength(controlPoints, precision = 1000) {
  let length = 0;
  ...
  for (let t = 0; t <= 1; t += 1 / precision) {
    ...
    if (previousPoint) {
      length += currentPoint.dist(previousPoint);
    }
...
function sCylinder(x, y, z, r, h, c) {
  push();
...
/*
//-28.598941368078158, -6, -61, -39.99185667752448, 1.4
let vpX = -16;
let vpY = -77;
let vpZ = 0;
let vpS = 1.6;
...
function handleviewCam() {
  if (vpMXOld != -1) {
    translate(vpX, vpY);
    rotate(radians(vpR1), [1, 0, 0]);
    rotate(radians(vpR2), [0, 1, 0]);
    rotate(radians(vpR3), [0, 0, 1]);
    scale(vpS);
    scale(-1, 1, -1);
    rotate(radians(180));
    //rotate(radians(mouseX), [0, 1, 0]);
    //  scale(mouseY/100);
    if (
      mouseX > 0 &&
      mouseY > 0 &&
      ...
      if (mouseIsPressed) {
        if (keyCode === SHIFT && keyIsPressed && vpMXOld != -1) {
          vpX = vpX + mouseX - vpMXOld;
          vpY = vpY + mouseY - vpMYOld;
        } else {
          vpR2 = vpR2 + mouseX - vpMXOld;
          vpR1 = constrain(vpR1 - mouseY + vpMYOld, -90, -20);
        }
      } else {
        if (this._mouseWheelDeltaY !== 0) {
          vpS = constrain(vpS + this._mouseWheelDeltaY / 50, 0.7, 10);
...
let target;                  // point we orbit around
let theta = 2;               // azimuth (around Y)
let phi = 0.5;                 // elevation
let radius = 400;            // camera distance (dolly)
let fov = Math.PI / 3;       // field of view (FOV zoom)
const R_MIN = 50, R_MAX = 3000;
let FOV_MIN ;
...
function updateCamera() {
  ...
  {
     cameraSetup=true;
  // camera control- code is in helper.js
  
    FOV_MIN = radians(15), FOV_MAX = radians(90);
  ...
  //console.log(phi);
  // clamp elevation so we don’t flip the up vector
  phi = constrain(phi, 0, Math.PI / 2 - 0.001);
  radius = constrain(radius, R_MIN, R_MAX);

  // spherical -> Cartesian
  const cp = Math.cos(phi), sp = Math.sin(phi);
  const ct = Math.cos(theta), st = Math.sin(theta);

  const camX = target.x + radius * cp * st;
  const camY = target.y + radius * sp;
  const camZ = target.z + radius * cp * ct;

  // up vector stays world +Y
...
function mouseDragged() {
  ...
  if (keyIsDown(SHIFT)) {
    // --- PAN ---
    // move the target in camera-right and camera-up directions
    const panSpeed = radius * 0.002;
    const dx = -(mouseX - pmouseX) * panSpeed;
    const dy = -(mouseY - pmouseY) * panSpeed;

    // camera-right in world space (per current theta)
    const rightX =  Math.cos(theta);
    const rightZ = -Math.sin(theta);

    target.x -= dx * rightX;
    target.z -= dx * rightZ;
    target.y += dy; // world up
  } else {
    // --- ORBIT ---
    const rotSpeed = 0.01;
    theta -= -(mouseX - pmouseX) * rotSpeed;     // around Y
    phi   -= -(mouseY - pmouseY) * rotSpeed;     // elevation
  }
}
...
function mouseWheel(e) {
  ...
  if (keyIsDown(ALT)) {
    // --- FOV zoom ---
    fov = constrain(fov + e.delta * 0.001, FOV_MIN, FOV_MAX);
  } else {
    // --- Dolly (true zoom) ---
    radius += e.delta; // positive delta -> farther
  }
