...
function draw() {
  ...
  if (showHelp) {
    background("white");
    let ratio = imgGuideJS.height / imgGuideJS.width;
    let iW = width;
    let iH = width * ratio;

    image(imgGuideJS, 0, 100, iW, iH);
    ...
    for (let i = 0; i < tmpSession.items.length; i = i + 1) {
      if(!playBack || (playBack && millis() -playBackTimer > tmpSession.items[i].time - tmpSession.startTime))
      {
      let posX =
        100 +
      ...
      if (
        uiCircle("", 0, 10, 10, posX, 200 + tmpSession.items[i].type * 15)
          .hover &&
        !hoverOn
       || (playBack && i < tmpSession.items.length-1 && millis() -playBackTimer < tmpSession.items[i+1].time - tmpSession.startTime)) {
        hoverOn = true;
        stroke("red");
